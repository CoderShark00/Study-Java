...
/*
    ...
    추상 클래스
    1. 추상 메서드가 하나라도 포함되면 해당 클래슨느 추상 클래스로 정의돼야 한다.
    ...
    추상 클래스의 목적
        ...
        2. 구체적인 구현 강제 : 추상 메서드를 통해 서브 클래스가 반드시 특정 메서드를 구현하도록 강제할 수 있따.
        3. 코드의 재사용성 : 중복되는 코드를 줄이고 유지 보수성을 높일 수 있다.

    추상 클래스에서 주의할 점 :
        - 추상 클래슨느 객체를 생성할 수 없다.
...
public class Main {
    public static void main(String[] args) {
        ...
        TabletFactory tabletFactory1 = new TabletFactory("애플 테블릿 공장");
        ...
        phoneFactory1.manage(); // Factory.java에 있는 추상 메서드 manage를
                                // orverride해서 재정의한 PhoneFactory만의 메서드
        tabletFactory1.manage();
        System.out.println();
        phoneFactory1.printInfo(); // 슈퍼 클래스의 일반 메서드를
        tabletFactory1.printInfo(); // 서브 클래스의
